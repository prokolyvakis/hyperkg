...
void getFrequencies(INT *extFreqEnt, INT *extFreqRel) {
	for (INT i = 0; i < entityTotal; i++)
		extFreqEnt[i] = freqEnt[i];
	for (INT i = 0; i < relationTotal; i++)
		extFreqRel[i] = freqRel[i];
}
...
void importTrainFiles() {
	...
	exRels = new set<INT>[entityTotal];
	for (int i = 0; i <= entityTotal - 1; i++) { 
		exRels[trainList[i].t].insert(trainList[i].h);
		exRels[trainList[i].h].insert(trainList[i].t);
	}
}
